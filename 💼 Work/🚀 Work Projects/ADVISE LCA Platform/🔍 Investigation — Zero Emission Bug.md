...
---
> **Status:** In Progress | **Area:** LCA calculation engine → indicator coverage gaps
...
The LCA calculator builds **one matrix** with ALL indicator UUIDs (union of Emission + CF sources). Each exchange only contributes to indicators covered by its tagged EF/CF dataset. Exchanges that are:
...
### ✅ Step 1 — Confirmed all exchanges have `upr_impact_assessment_settings` populated
...
### ✅ Step 2 — Checked EF coverage per exchange for a specific indicator
...
**Finding:** `ef_value` returned NULL for Emission-type exchanges for indicator `3c29b49e-...` — EF dataset doesn't cover this indicator.

### ✅ Step 3 — Confirmed `lcia_description` row exists for the `ref_id`

```sql
SELECT * FROM lca.lcia_description WHERE id = 'd0c823c0-4959-4b21-9c8b-f0484a9cdf38';
```

**Finding:** Row exists — the `ref_id` is valid.

### ✅ Step 4 — Checked `lcia_data` coverage for that description + indicator
...
**Finding:** No rows returned — **the data gap is in **`**lcia_data**`** itself.** The EF dataset for this description does not include this indicator.
...
| Field | Value |
| --- | --- |
| Product / stage `lcia_description_id` | `87c9c059-1a27-42b8-affa-5297204734eb` |
| EF description `ref_id` | `d0c823c0-4959-4b21-9c8b-f0484a9cdf38` |
| Indicator under investigation | `3c29b49e-9e0d-488c-ae3a-368ebe6ecf25` |
...
- [ ] Establish whether this is a data import gap or an expected limitation of the EF dataset
- [ ] Investigate whether the EF dataset simply doesn't cover this indicator (e.g. it only exists in CF datasets like IPCC 2021)

---

## Debugging Plan — ecoinvent ecoquery

**Source:** [https://ecoquery.ecoinvent.org/3.9.1/cutoff/files](https://ecoquery.ecoinvent.org/3.9.1/cutoff/files)

Using two datasets to debug and cross-validate indicator coverage:

| Dataset | Relationship | Purpose |
| --- | --- | --- |
| **Allocation cut-off by classification** | One-to-many (`lcia_id` ↔ `upr_exchange_name_id`) | Primary dataset for coverage gap investigation |
| **Substitution, consequential, long-term** | One-to-one (`lcia_id` ↔ `upr_exchange_name_id`) | Simpler mapping — use to validate and test |

**Approach:** Check whether the missing indicator (`3c29b49e-...`) exists in either dataset's EF coverage via ecoquery. If present in one but not the other, the issue is dataset-specific rather than a platform import bug.