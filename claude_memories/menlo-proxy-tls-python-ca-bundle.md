...
metadata: 
  ...
  modified: 2026-08-31T09:29:08.128Z
---
...
**SEPARATE failure, same proxy — `curl` and the PROXY ENV VARS (found 2026-08-31).** The CA bundle above fixes *verification*; it does NOT fix *routing*. `HTTP_PROXY`/`HTTPS_PROXY` are set to `http://proxy0-<hash>.menlosecurity.com:3131` with `NO_PROXY=localhost,127.0.0.1,::1,.local`, so every `curl` to a public host is routed through Menlo and returns **exit 0 with HTTP code `000`** (no connection) — including `https://example.com`. Adding `--noproxy '*'` bypasses it and the same request succeeds: `curl -s --noproxy '*' https://api.dev.identity.advise.technology/health` → **200 `ok`**. So the eTEA primer's advice ("this network's TLS interception breaks the AWS CLI and `curl` — verify from workflow logs or the in-app browser instead") is **incomplete and misleading for curl**: curl is not broken, it is proxied, and `--noproxy '*'` restores it. Reach for that before falling back to the browser. (Matches the `curl --noproxy` pattern already noted for ce2m.) AWS CLI is a different story — it fails on expired SSO tokens, not the proxy.
