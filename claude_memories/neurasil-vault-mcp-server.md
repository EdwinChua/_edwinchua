...
metadata: 
  ...
  modified: 2026-08-05T09:40:26.429Z
---
...
**NEXT (user's stated direction 2026-08-05):** build a **Notion-like web UI** around the vault to view/edit notes+tables visually. The design already anticipates this — core `src/neurasil/vault/` is MCP-agnostic behind a thin adapter (`tools.py`), so the UI attaches via a NEW REST adapter over the same core; stable UUIDs + `parent`/`order`/`icon` metadata + the `relation` type are already in place for it. **DEFERRED:** Notion→vault data migration (Notion connector is working again, so no rush); re-point `dbs-statement-reconciliation` skill at neurasil.
