---
tags: [hub, cooking, recipes, suppliers]
---
Recipes and cooking notes — dishes worth making again. Each recipe links back to the database below; click any one for the full ingredients and method.

---

## 📚 What's in here

- 🦆 **Duck Breast Prosciutto** — salt-cured, air-dried duck. Easy but slow (~8 days, mostly hands-off).

---

## 🛒 Suppliers

Where to source good ingredients.

- 🥩 **[Tasty Food Affair](https://www.tastyfoodaffair.com/)** — local supplier, good value for steaks. ⏳ Requires **~3 days** to prepare the order, so plan ahead. #supplier #beef #local
	- *Reference price:* **New Zealand Fresh Grass-Fed Ribeye** — 4 × 280g for **SGD $51.80** (≈ $12.95/steak, ≈ $46/kg).

---

> 💡 **Tip:** Log a new recipe with the `recipe` tag and set its `base` to `[[🍳 Cooking.base]]` so it shows up in the table below. Use `Category`, `Difficulty`, and `Total Time` so you can sort/filter when deciding what to cook.

![[🍳 Cooking.base]]

[[🦆 Duck Breast Prosciutto]]

[[🧠 Second Brain]]
